...
vLines <- data.frame( stat_type=c("Mean", "Median"), vals=c( meanSteps, medianSteps ))
g <- g + geom_vline( data = vLines, aes( xintercept=vals, colour = stat_type ), 
                     show_guide = TRUE )
print( g )
```
...
sprintf( "The median number of steps: %d.", medianSteps )
```
...
```
```{r, echo=FALSE}
sprintf( "Max number of steps interval of the day: %02d:%02d", floor( maxNumStepsPerInt/100 ), maxNumStepsPerInt %% 100 )
```
...
```
```{r, echo=FALSE}
sprintf( "The number of rows with NAs: %d", numDataRowsNas )
```
...
g <- g + labs( title="Total Number of Steps per Day", y="Frequency", 
               x="Total Steps in a Day" )
vLines <- data.frame( stat_type=c("Mean", "Median"), vals=c( meanStepsMeanMod, medianStepsMeanMod ))
g <- g + geom_vline( data = vLines, aes( xintercept=vals, colour = stat_type ), 
                     show_guide = TRUE )
print( g )
```
...
```{r, echo=FALSE}
sprintf( "The mean number of steps with the unmodified data is: %.02f.", meanSteps )
sprintf( "The median number of steps with the unmodified data is: %d.", medianSteps )
# report the mean and median total number of steps taken per day with the mean modified data
sprintf( "The mean number of steps with the mean modfied data: %.02f.", meanStepsMeanMod )
sprintf( "The median number of steps with the mean modified data: %.0f.", medianStepsMeanMod )
```
